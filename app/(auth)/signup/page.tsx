...
import { signUp, signInWithGoogle, signInWithGithub } from "@/lib/firebase/auth";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { useToast } from "@/hooks/use-toast";
import { GraduationCap, Mail, Lock, User, ArrowRight, CheckCircle2, Users, Briefcase, Sparkles } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Default role as "student" - will be updated during onboarding
      ...
      const errorMessage = error.message || "Failed to create account. Please check your Firebase configuration.";
      toast({
        title: "Error",
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Don't pass role - will be set during onboarding
      ...
      if (result) {
        ...
        toast({
          title: "Success",
          description: isNewUser ? "Account created with Google!" : "Signed in with Google!",
        });
  ...
  const handleGithubSignIn = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        toast({
          title: "Success",
          description: isNewUser ? "Account created with GitHub!" : "Signed in with GitHub!",
        });
        // New users go to onboarding, existing users go to dashboard
        const destination = isNewUser ? "/onboarding" : "/dashboard";
        console.log("Redirecting to:", destination);
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-lg text-gray-600">
              Join thousands of students and alumni
            </p>
          </div>
      ...
      <div className="hidden lg:flex flex-1 bg-linear-to-br from-gray-900 via-gray-800 to-black p-12 items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg space-y-8 text-white">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full border border-white/20">
            ...
            <span className="text-sm font-medium">Free to join • No credit card required</span>
          </div>
          ...
          <p className="text-xl text-gray-300 leading-relaxed">
            Get instant access to verified alumni mentors, exclusive job postings, and a thriving community of students from top institutions.
          </p>

          <div className="space-y-6 pt-8">
            {[
              { icon: CheckCircle2, text: "Connect with 1,000+ verified alumni mentors" },
              { icon: Briefcase, text: "Access to 500+ exclusive job opportunities" },
              { icon: Users, text: "Join 10,000+ students already networking" },
              { icon: Sparkles, text: "AI-powered mentor matching system" },
            ].map((benefit, idx) => (
              <div key={idx} className="flex items-start gap-4">
                ...
                <p className="text-lg text-gray-300 pt-1.5">{benefit.text}</p>
              </div>
            ))}
          </div>
        </div>
